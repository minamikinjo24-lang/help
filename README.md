...
`Dockerfile` はビルド工程を持たず、本番依存の導入と起動だけを行います。
`fly.toml` は Fly.io 用の例で、秘密値は含めていません。

**SQLite を使っているため、永続ボリュームが必須です。** マウントせずに公開すると
デプロイのたびにデータが消えます。同じ理由で、インスタンスは常に1台に固定します。
