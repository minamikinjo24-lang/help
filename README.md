...
| 13 | `PATCH /api/tickets/:id` | 401 | 403 | 200 | 200 |
...
「この分類案を採用して保存する」を押すと `PATCH /api/tickets/:id` で保存されます。
押すまでは何も変わりません。このボタンは agent / admin にだけ出ます。
...
タイトルと本文の編集、検索、添付ファイル、装飾、ロール変更画面、担当者の割り当て。
